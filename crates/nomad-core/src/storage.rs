...
impl NomadContentStore {
    ...
    pub fn list_pages(&self) -> Result<Vec<NomadPageEntry>, NomadError> {
        let mut out = Vec::new();
        collect_files(&self.roots.pages_dir, &self.roots.pages_dir, &mut out)?;
    ...
    pub fn list_files(&self) -> Result<Vec<NomadPageEntry>, NomadError> {
        let mut out = Vec::new();
        collect_files(&self.roots.files_dir, &self.roots.files_dir, &mut out)?;
        out.sort_by(|a, b| a.path.cmp(&b.path));
        Ok(out)
    }
    ...
    pub fn read_page_rel(&self, rel: &str) -> Result<Vec<u8>, NomadError> {
        ...
        if bytes.len() > self.roots.max_page_bytes {
    ...
    pub fn read_file_rel(&self, rel: &str) -> Result<Vec<u8>, NomadError> {
        ...
        if meta.len() as usize > self.roots.max_file_bytes {
            return Err(NomadError::TooLarge {
...
fn collect_files(root: &Path, dir: &Path, out: &mut Vec<NomadPageEntry>) -> Result<(), NomadError> {
    if !dir.exists() {
        return Ok(());
    }
